...
export default function BillingRow({ row, splitMode, onToggle, onHoursChange }: Props) {
  return (
    <div
    ...
    >
      ...
      <label className="block">
        <span className="mb-1 block text-xs font-black uppercase">Số giờ</span>
        <input
          type="number"
          min={0}
          step={0.5}
          ...
          onChange={(event) => onHoursChange(row.voterId, Number(event.target.value || 0))}
      ...
      <div className="flex items-end justify-between gap-3 md:block">
        <span className="text-xs font-black uppercase">Tạm tính</span>
        <p className="text-xl font-black">{currencyFormatter.format(row.amount)}</p>
      </div>
    </div>
  )
}
